...
if (w) {
  test('副本称号', 'LV.0 返回第一个称号(Lv.1)', getCurrentTitle(w, 0)?.name, '拾荒者');
  test('副本称号', 'LV.1 = 拾荒者', getCurrentTitle(w, 1)?.name, '拾荒者');
  test('副本称号', 'LV.2 = 流浪者', getCurrentTitle(w, 2)?.name, '流浪者');
  test('副本称号', 'LV.3 = 幸存者', getCurrentTitle(w, 3)?.name, '幸存者');
...
test('副本成就', 'Lv.1 解锁1个(首个称号titles)', checkScenarioAchievements(emptyGs, w, 1).length, 1);
test('副本成就', 'Lv.5 解锁初踏废土+首个称号+拾荒者之证', checkScenarioAchievements(emptyGs, w, 5).length, 3);
test('副本成就', 'Lv.10 解锁初踏废土+十级之路+首个称号+拾荒者之证', checkScenarioAchievements(emptyGs, w, 10).length, 4);
test('副本成就', 'Lv.500+无runtime/events 解锁7个(minRebirth=0)', checkScenarioAchievements(emptyGs, w, 500).length, 7);
// runtime 条件
const rtGs = { ...emptyGs, totalRuntimeMs: 3600000 };
test('副本成就', 'runtime=1h 解锁1h成就', checkScenarioAchievements(rtGs, w, 1).some(a => a.id === 'wasteland_a005'), true);
// events 条件
const evGs = { ...emptyGs, triggeredEvents: Array(50).fill('x') };
test('副本成就', 'events=50 触发大量事件成就', checkScenarioAchievements(evGs, w, 1).length > 0, true);
// 已解锁的不重复
const unlockedGs = { ...emptyGs, unlockedAchievements: ['wasteland_a001', 'wasteland_a011'] };
test('副本成就', '已解锁成就不重复(Lv.5初踏废土+首个称号已解锁=剩1)', checkScenarioAchievements(unlockedGs, w, 5).length, 1); // wasteland_a012 拾荒者之证

...
| **打字给经验** | 每按键 +0.3 EXP，连击段位越高倍率越高（D〜SSS 从 x1.2〜x20），加速升级 |
| **Buff 爆发** | 高速打字维持 10 秒触发 buff（B→x2 持续 60 秒 / A+→x3 持续 120 秒），冷却 30 秒 |
| **打字陪伴** | 全局按键监听\*，宠物头顶随机弹出按下的按键字符 + 连击数，段位系统内部驱动 EXP 倍率 |
...
| **跨平台** | Windows / macOS / Linux 均可正常使用 |

## 截图

| 大厅界面 | 副本内挂机 | 宠物窗口 |
|---------|-----------|---------|
| ![大厅](docs/screenshots/hub.jpg) | ![副本内](docs/screenshots/scenario.jpg) | ![宠物](docs/screenshots/pet-window.jpg) |

| 事件弹窗 | 打字连击 | 存档同步 |
|---------|---------|---------|
| ![事件弹窗](docs/screenshots/event.jpg) | ![打字连击](docs/screenshots/combo-hit.gif) | ![存档同步](docs/screenshots/sync-setting.jpg) |
...
游戏依赖 `WinKeyServer.exe`（位于安装目录）实现全局按键监听（打字连击反馈功能）。部分杀毒软件（尤其是 Windows Defender）可能会将其误报为威胁并自动隔离。

- 如果打字功能不生效，请检查 Windows 安全中心 → 病毒和威胁防护 → 保护历史记录，找到被隔离的 `WinKeyServer.exe` 并点击**还原**
- 还原后建议添加排除项：Windows 安全中心 → 管理设置 → 排除项 → 添加排除项 → 选择文件 → 找到 `WinKeyServer.exe`，避免再次误杀
- 此为误报，该文件来自开源库 [node-global-key-listener](https://github.com/nicekingwei/node-global-key-listener)，仅用于监听键盘输入实现游戏内打字反馈，不会记录或上传任何按键数据
...
4. 游戏会自动在云盘里创建 `Idel-DreamMaker-Sync` 文件夹，包含存档和日志

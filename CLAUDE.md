...
| **2.7.4** | **2026-06-20** | **副本生命周期管理：** 删除不完整的 hermit/starfield 副本；新增归档功能(所有周目通关后可归档隐藏+已归档折叠区)；新增全部通关催更入口(大厅提示→打开设置面板，一次性消除)；新增设置面板反馈区块(邮箱复制+导出日志到桌面+打开日志文件夹)。—— **第二次更新：** 新增大厅挂机提醒(5分钟触发+15分钟间隔，全部通关后不提醒，进副本重置)。 |
...
```
输出到 `release/` 目录，Windows 约 150MB `.exe` 便携版。
...
| 生产打包 | `npm run electron:build` | 构建副本数据 + Vite build + electron-builder（Windows 约 150MB） |

...
| `scripts/generate-icon.mjs` | 图标生成 | 打包前从 icon.png 生成合法 BMP 格式 .ico |
...
| `themes/style-guide.md` | 样式规范 | 全项目 CSS 统一规范（色彩/字体/间距/动画/符号） |
| `start.bat` | 启动脚本 | Windows 双击启动开发模式（等同 `npm run electron:dev`） |
| `update_hermit.ps1` | 副本工具 | 生成 hermit.md 的 filler 事件（作者工具，PowerShell） |
| `.opencode/` | opencode 配置 | 项目级 AI 工具配置目录 |
| `.gitignore` | Git 忽略规则 | 忽略 node_modules/dist/字体等 |
| `vite.config.js` | Vite 配置 | 开发端口 1420 等 |
| `package.json` | npm 配置 | 版本号、脚本、依赖、electron-builder 配置 |
...
| 项目 | 数值 | 说明 |
|------|------|------|
| EXP 速率 | 1 EXP/s 基础 | 按挂机变体机制调整：cultivation 白天1.5x夜晚0.7x；cyber 故障暂停+补偿2x；tide 每6h前10min2x；polar 冬季0.5x |
| 大厅等级公式 | `floor(sqrt(hub_total_exp / 100)) + 1` | 全局进度，永不重置 |
| 副本等级公式 | LV1-100: `floor(sqrt(exp/100))+1`；LV100+: `100 + floor((exp-980100)/6000)` | 分段加速，LV500 约117天（8h/天） |
| 称号 | 30 个/副本 + 20 个大厅等级称号 + 通关称号 | 副本称号分布在1-500级；大厅称号代码内置LV1-500 |
| 事件文本 | story ~500/周目×(1+max_rebirth) 条/副本 + filler ~1700/周目×(1+max_rebirth) 条/副本 | 每周目500主线+1700日常；filler 各周目独立，每周目约1700条 |
| 成就 | 50 个/副本 + 10 个大厅成就 | 副本成就四类均衡；大厅成就含等级/通关/重生类 |
| 节日事件 | ~50 条/副本 | ~25 个节日，每个 advance + day 各 1 条 |
| 里程碑事件 | 8 条（代码内置） | 1h/6h/24h/3d/7d/30d/100d/365d，跨副本通用，只触发一次 |
| 每日仪式 | 1 条（代码内置） | 每天首次登录触发，当日 filler 上限 +2 |
| 日志保留 | 不限 | 事件持久化到按日文件 + 内存保留，界面滚动查看 |
| 存档 | 单文件 JSON | 全局 `save.json`，内含 `scenarioProgress` 各副本进度嵌套 |

**大厅称号：** 20 个等级称号（代码内置 LV1-500）+ 通关称号（每副本 1 个，写在 .md completion_title）。各副本已解锁称号的聚合视图，按副本分组，UI 可展开/折叠。
...
| 18. 日志持久化+历程菜单 | **100%** | v2.6.0 — 事件持久化、历程子菜单、事件历史面板、页面滚动条统一 |
| 19. 游戏架构大升级 | **100%** | v2.7.0 — 等级曲线分段/里程碑/每日仪式/结局重生/5种挂机变体/大厅称号成就/解锁框架 |
| 20. 配色+UX优化 | **100%** | v2.7.1 — 配色优化/宠物背景/bubble修复/报时自动展开/UX优化 |
| 21. UI/UX 专业度提升 | **100%** | v2.7.2 — 6项优化（行距/scrollbar-gutter/z-index/骨架屏/will-change/font-display） |
| 22. ScenarioWriter 升级 | **100%** | v2.7.3 — 第二人称/分段生成/灯停规则/validate_md.cjs自检/build.js报错 |

**当前阶段：** v2.7.5 — 宠物新手指引优化+启动默认显示+bug修复。宠物窗口三段式渐进引导（无宠物指引面板→首次操作提示→右键操作说明）。启动自动显示主窗口。修复宠物窗口不显示的竞态条件、有宠物用户跳过引导、pet-bubble滚动条样式、主窗口焦点被抢后瞬间关闭。
...
| **v2.5.1** | **Bug修复：关闭按钮/升级合并/pet气泡/报时改版** | ✅ |
| **v2.6.0** | **日志持久化+历程菜单+事件历史面板+样式规范+CSS统一** | ✅ |
| **v2.7.0** | **游戏架构大升级：等级曲线分段/里程碑/每日仪式/结局重生/5种挂机变体/大厅称号成就/解锁框架** | ✅ |
| **v2.7.1** | **配色优化+宠物背景修复+bubble闪烁修复+报时自动展开+UX优化** | ✅ |
| **v2.7.2** | **UI/UX 专业度提升（6项优化）** | ✅ |
| **v2.7.3** | **Bug修复 + completion_title 实装 + ScenarioWriter 对齐** | ✅ |
| **v2.7.4** | **副本生命周期管理（归档+催更+反馈）** | ✅ |
| **v2.7.5** | **宠物新手指引+启动显示+bug修复** | ✅ |
| v2.8+ | Mac 适配 / Linux 打包 / Steam / 新副本 | 待定 |
...
| **2.5.1** | **2026-06-18** | **Bug修复+报时改版：** 关闭按钮绑定、story事件升级合并修复、pet气泡合并故事文本、报时从infoBar改nq气泡、半小时报时、去掉主窗口清队列限制、log-area补故事文本 |
| **2.6.0** | **2026-06-18** | **日志持久化+历程菜单+事件历史面板+样式规范+全CSS统一：** 事件持久化到按日文件、历程子菜单(称号/成就/事件)、事件历史面板日期分组+懒加载、启动加载今日事件、全部滚动条统一样式、pet通知队列排队+bubble-closed触发下一条、pet选择器样式统一(箭头/滚动条)、样式规范文档(themes/style-guide.md)、全CSS统一(--border变量/color-mix替换固定rgba/动画规范/:active统一0.96/关闭按钮统一14px/字体变量/透明度0.9→0.85/E0E0E0→color-mix)、版本管理章节、版本号2.5.0→2.6.0 |
| **2.7.0** | **2026-06-18** | **游戏架构大升级：** 等级曲线分段加速(LV100+线性,LV500从864天→117天)；filler动态上限(8+floor(挂机h/4)+每日仪式2+重生加成)；8条里程碑事件(1h/6h/24h/3d/7d/30d/100d/365d,白话文本,只触发一次)；每日仪式(固定文本,当日filler+2)；chime报时优先级0→0.5；副本结局+重生(LV500自动弹特殊弹窗,无限重生,经验+10%封顶+50%,filler+5封顶+25)；5种挂机变体机制(standard/cultivation/cyber/tide/polar)；重生番外框架(Events表MinRebirth列,四周目story结构)；20个大厅等级称号(代码内置LV1-500)；10个大厅成就(等级类+通关数量类+重生类)；大厅界面显示称号+通关统计；称号/成就面板分组(副本/大厅)；副本解锁框架(unlock_requirement字段+锁定副本UI)；结局弹窗+结局收藏面板(历程菜单第4项)；状态栏重生标识(R1/R2)；build.js解析新frontmatter字段+MinRebirth列；CLAUDE.md新增长期规划与扩展路线章节；ScenarioWriter skill完整升级(四周目结构+5种机制推荐+游戏机制说明) |
| **2.7.1** | **2026-06-18** | **配色优化+宠物背景修复+bubble闪烁修复+报时自动展开+UX优化：** 删除6个低对比度主题保留9个(对比度≥10:1,fg更亮bg更深dim拉开层次)；白色主题特殊语义色(成就金改深金/青色改深青)；宠物info-bar/exp-wrap改默认淡显示(opacity:0.3)+悬浮清晰(:hover opacity:1)不加背景色保持完全透明；text-shadow辅助可见性；pet bubble闪烁修复(doShowBubble先更新内容再显示避免闪旧内容)；报时改为直接弹bubble+6秒自动消失不进dot队列；删除抽取副本功能(按钮+IPC全部移除)；状态栏精简(版本号移到设置面板)；副本卡片加进度(通关✓+重生R数/进行中Lv.X)；大厅引导提示(未进过副本时)；EXP进度条加粗(6px→10px)；标题栏升级闪烁(level-up时LV数字闪烁变色+缩放动画)；THEMES数组同步删除6主题+更新配色值 |
| **2.7.2** | **2026-06-18** | **UX优化（基于UI/UX Pro Max skill）：** 阅读类文本行距加宽(事件/结局/引导/弹窗描述 line-height 1.6-1.7)；侧面板加 scrollbar-gutter:stable 稳定布局；事件弹窗 z-index 100→250、成就弹窗 101→251 不被面板遮挡；骨架屏加载(呼吸闪烁占位条替代"加载中...")；7组动画元素补 will-change 减少掉帧；5个HTML的@font-face加 font-display:swap 避免启动瞬时空白 |
| **2.7.3** | **2026-06-19** | **废土副本重写 + 游戏Bug修复 + ScenarioWriter优化：** 废土副本升级为4周目完整格式(2000story+6782filler+30称号+50成就+52节日)；修复preload白名单缺失8通道导致大厅功能/结局弹窗/重生失效；修复重生上限检查和通关称号UI刷新；补R0 filler至1700对齐密度表；ScenarioWriter skill重写(串行分步+大纲先行+ID分配表+build.js常见失败表+完整frontmatter模板+备份指引+性能预期)；清理工作区旧脚本和文档引用。—— **第二次更新：** ScenarioWriter skill再加5项(第二人称/分段生成/灯停规则/validate_md.cjs自检/Action说明/BOM防污染)；build.js parseTable加精准报错；写validate_md.cjs格式自检脚本；回退starfield+hermit保持废土独版。 |
| **2.7.4** | **2026-06-20** | **副本生命周期管理：** 删除不完整的 hermit/starfield 副本；新增归档功能(所有周目通关后可归档隐藏+已归档折叠区)；新增全部通关催更入口(大厅提示→打开设置面板，一次性消除)；新增设置面板反馈区块(邮箱复制+导出日志到桌面+打开日志文件夹)。—— **第二次更新：** 新增大厅挂机提醒(5分钟触发+15分钟间隔，全部通关后不提醒，进副本重置)。—— **第三次更新：** 修复 exe 图标（新增 scripts/generate-icon.mjs 从 icon.png 生成合法 BMP 格式 .ico）；修复窗口任务栏图标（electron/windows.cjs 加 icon 选项）；Linux 打包改为 tar.gz（Windows 无法打 AppImage）；npm run electron:build 自动清理中间产物。 |
| **2.7.5** | **2026-06-21** | **宠物新手指引优化+启动默认显示+bug修复：** 宠物窗口三段式渐进引导（无宠物指引面板→首次操作提示→右键操作说明）。启动自动显示主窗口。修复宠物窗口不显示的竞态条件、有宠物用户跳过引导、pet-bubble滚动条样式、主窗口焦点被抢后瞬间关闭。 |

## 长期规划与扩展路线

> 本章节记录游戏长期架构规划和扩展路线，供新 AI 会话参考。
> 核心原则：本次架构一次性搭好，后续增长靠作者用 skill 产副本内容，不需要改代码。

### 架构能力（v2.7.0 搭建）

| 能力 | 说明 |
|------|------|
| 副本数量无上限 | build.js 扫描 scenarios/ 目录，多少个 .md 都能解析 |
| 挂机变体机制 | 5 种：standard/cultivation/cyber/tide/polar，写在 .md frontmatter |
| 重生番外 | 每个副本支持多周目 story（MinRebirth 列），max_rebirth 由作者设定 |
| 大厅解锁框架 | unlock_requirement 字段，作者按需设解锁条件 |
| 大厅称号 | 等级称号 20 个（代码内置 LV1-500）+ 通关称号（每副本 1 个，写在 .md completion_title） |
| 大厅成就 | 10 个（代码内置），含等级类和通关数量类 |
| 结局+重生 | LV500 自动结局，无限重生，经验加成封顶 +50%，filler 加成封顶 +25 |
| 里程碑事件 | 8 条白话文本（代码内置），跨副本通用，只触发一次 |
| 每日仪式 | 固定文本（代码内置），当日 filler 上限 +2 |
| filler 动态上限 | 8 + floor(挂机小时/4) + 每日仪式 2 + 重生加成 |

### 挂机变体机制

| 机制 | 代码名 | 行为 | 适合主题 |
|------|--------|------|---------|
| 标准 | `standard` | 1 EXP/s 恒定 | 废土、隐居、日常 |
| 修仙 | `cultivation` | 白天6-18点1.5x，夜晚0.7x | 修仙、武侠、仙侠 |
| 赛博 | `cyber` | 每10分钟5%概率故障暂停30秒，补偿60秒2x | 赛博、科幻、末日机器 |
| 潮汐 | `tide` | 每6小时周期，前10分钟2x经验 | 海岛、渔村、海洋 |
| 极昼 | `polar` | 现实季节：夏季正常，冬季0.5x | 极地、雪山、北极 |

### 副本增长路线

| 阶段 | 副本数 | 需要做的事 | 改代码吗 |
|------|--------|-----------|---------|
| 当前 | 3 | 用 skill 重新生成（带新字段） | 否 |
| 初期 | 5-8 | 自然增长，解锁框架就位 | 否 |
| 中期 | 8-15 | 副本列表 UI 优化（分类/筛选/搜索）；结局收藏册 UI | 改前端 |
| 后期 | 15-30 | 跨副本成就系统；年度回顾；副本推荐 | 改前端+主进程 |
| 远期 | 30+ | 副本资料片；多周目统计；社交功能（如上 Steam） | 大改 |

### 解锁曲线建议

作者用 skill 产副本时，按以下原则设 unlock_requirement：

| 副本序号 | 建议解锁条件 | 理由 |
|---------|-------------|------|
| 1-3 | 默认解锁 | 新手友好 |
| 4-6 | 大厅 LV30/50/80 | 几小时到几天挂机 |
| 7-10 | 通关 1/2/3/4 个副本 | 长期目标 |
| 11-15 | 大厅 LV100/120/150/180/200 | 硬核玩家 |
| 16+ | 通关 5/6/7... 个副本 | 资深玩家 |
...
超过 LV500 显示最高称号"万界漫游者"。

### 大厅成就（10 个，代码内置）

| 成就 | 条件 | 描述 |
|------|------|------|
| 初出茅庐 | 大厅 LV50 | 大厅等级达到 50 |
| 百级之路 | 大厅 LV100 | 大厅等级达到 100 |
| 两百之巅 | 大厅 LV200 | 大厅等级达到 200 |
| 三百之峰 | 大厅 LV300 | 大厅等级达到 300 |
| 集邮者 | 通关 3 个不同副本 | 收集了三个世界的结局 |
| 半数圆满 | 通关当前一半副本 | 走过半数旅程 |
| 大圆满 | 通关所有副本 | 所有世界皆已圆满 |
| 重生者 | 任意副本重生 1 次 | 再来一次 |
| 轮回者 | 任意副本重生 3 次 | 三世轮回 |
| 万界旅人 | 通关 5 个不同副本 | 五个世界的旅人 |
...
| UI 项 | 说明 |
|-------|------|
| 结局弹窗 | 全屏遮罩+中央卡片，副本名+结局文本+重生/返回按钮，金色边框 |
| 锁定副本显示 | 灰色半透明卡片+锁定提示+解锁条件，不可点击 |
| 大厅称号显示 | 大厅界面显示当前大厅称号 |
| 大厅信息补充 | 大厅界面加通关统计（X/Y 副本已通关） |
| 称号面板分组 | "副本称号"和"大厅称号"两个分组 |
| 成就面板分组 | "副本成就"和"大厅成就"两个分组 |
| 历程菜单扩展 | 4 项：称号/成就/事件/结局 |
| 状态栏重生标识 | 副本内 LV 后加(R1)/(R2)表示重生次数 |

#### 后续 UI（副本增多时）

| UI 项 | 时机 | 说明 |
|-------|------|------|
| 副本列表筛选标签 | 副本 8+ | 全部/进行中/已通关/未解锁 |
| 副本搜索框 | 副本 10+ | 按名称搜索 |
| 副本卡片状态标识 | 副本 8+ | 通关/重生中/锁定 |
| 副本卡片进度条 | 副本 8+ | 显示当前等级/500 |
| 结局收藏册完整 UI | 副本 5+ | 网格展示所有副本结局 |
| 年度回顾 UI | 长期 | 12.31 自动弹出 |
| 挂机变体机制提示 | 本次搭框架 | pet 窗口或状态栏显示当前机制 |

#### 不做的 UI 改动

| 改动 | 不做的原因 |
|------|-----------|
| 加宽窗口 | 320 瘦长是游戏特色，副本多了靠滚动解决 |
| 加图片/图标 | 游戏不做美术，纯文字+符号 |
| 复杂动画 | 保持简洁，动画只用于反馈 |
...
| 作者后续要做的事 | 怎么做 |
|----------------|--------|
| 产新副本 | 用 skill 生成 .md 放进 scenarios/ |
| 设解锁条件 | 写在 .md frontmatter unlock_requirement |
| 设通关称号 | 写在 .md frontmatter completion_title |
| 设挂机机制 | 写在 .md frontmatter mechanic |
| 设重生次数 | 写在 .md frontmatter max_rebirth |

### 需要改代码的扩展（可选增强）

| 想做的事 | 改动位置 | 何时做 |
|---------|---------|--------|
| 加大厅称号（LV500+） | main.cjs 常量数组 | 玩家大厅等级超 500 时 |
| 加新挂机机制 | main.cjs 游戏循环分支 | 有新主题需求时 |
| 加跨副本成就 | main.cjs 成就定义 | 副本 15+ 时 |
| 副本列表分类筛选 | src/main.js + style.css | 副本 8+ 时 |
| 结局收藏册 UI | src/main.js + style.css | 副本 5+ 时 |

### skill 作为增长引擎

- 作者用 ScenarioWriter skill 持续产副本
- skill 已支持：mechanic 推荐、max_rebirth 设定、unlock_requirement、completion_title、多周目 story 结构
- 每个副本 = 一次完整人生体验（每周目 story 500 + filler ~1700，max_rebirth=3 时合计 ~8200+ 条事件）
- 创作节奏建议：每周 1-2 个新副本
...
- **格式**：SemVer — `主版本.功能版本.修复版本`
- **动态显示**：`electron/main.cjs` 启动时读取 `package.json` 的 `version`，通过 IPC 传给前端显示，不写死
- **内部开发**：版本号正常迭代，不刻意归零
- **首次上架**：上架当天将 `package.json` version 改为 `1.0.0`，之后从 `1.0.0` 开始计数

### 什么时候改版本号
| 场景 | 动作 | 举例 |
|------|------|------|
| 修复 bug | 升修复版本 | `2.6.0` → `2.6.1` |
| 增加功能 | 升功能版本 | `2.6.0` → `2.7.0` |
| 大改版 | 升主版本 | `2.6.0` → `3.0.0` |
| 首次发布 | 改为 `1.0.0` | 上架日改 |

### 发布打包
| 命令 | 说明 |
|------|------|
| `npm run electron:build` | Windows 打包 → `release/Idel-DreamMaker 版本.exe`（便携版 ~85MB）|
| `npx electron-builder --linux` | Linux 打包 → `release/idel-dream-maker-版本.tar.gz`（可在 Windows cross-compile）|

### 发布前需确认
1. `scenarios/` 下副本内容已完成
2. `package.json` version 已更新
3. `icons/icon.ico` 存在
4. 执行打包后，双击 `.exe` 测试安装和运行
...
| 启动应用 | `npm run electron:dev` | 先构建副本数据（build.js）再 Vite + Electron 热重载 |
| 构建副本数据 | `node build.js` | .md → scenarios_data.json |
| 生产打包 | `npm run electron:build` | 构建副本数据 + Vite build + electron-builder（Windows 便携版 ~85MB） |
| Linux 打包 | `npx electron-builder --linux` | 生成 `release/idel-dream-maker-版本.tar.gz`（可在 Windows cross-compile）|
...
- Linux 打包（Ubuntu/Debian/Fedora）需在 Linux 环境下执行 `electron-builder --linux`，生成 `.deb` / `.AppImage`；在 Windows 上 cross-compile 生成 `tar.gz`

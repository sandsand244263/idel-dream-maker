...
| **v2.6.0** | **日志持久化+历程菜单+事件历史面板+滚动条统一** | ✅ |
...
| **2.6.0** | **2026-06-18** | **日志持久化+历程菜单+事件历史面板：** 事件持久化到按日文件、历程子菜单(称号/成就/事件)、事件历史面板日期分组+懒加载、启动加载今日事件、全部滚动条统一样式、pet通知队列排队+bubble-closed触发下一条、pet选择器样式统一、样式规范文档+全CSS统一(--border/color-mix/动画/:active/关闭按钮/字体变量/透明度) |

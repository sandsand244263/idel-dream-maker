...
**当前阶段：** v2.7.3 — 修复等级曲线系数 + 实现 completion_title 通关称号 + ScenarioWriter 完整对齐。下一阶段：用 skill 重新生成副本内容（带新字段）。
...
| **2.7.3** | **2026-06-19** | **Bug修复 + completion_title 实装 + ScenarioWriter 对齐：** 修复等级曲线 LV100+ 系数30000→6000（LV500从450天回归117天）；实现 completion_title 通关称号系统（通关自动解锁/大厅面板装备/覆盖等级称号）；ScenarioWriter skill 基于新曲线重写 filler 公式 floor(分钟/30)、4周目独立 filler、防重复5条规范、Action 列跟踪；同步修复前端 calcLevel/calcExpForLevel。 |

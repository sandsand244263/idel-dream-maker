...
1. **所有事件文本不在代码里硬编码**，由 AI 批量生成后放入副本文件（`.md` 格式，详见`副本MD格式模板.md`）
2. **游戏是零交互的**——进入副本后没有按钮、点击、选择分支。纯挂机 + 偶尔阅读
3. **Steamworks 集成为可选项**，延后到 1.0，代码里条件编译
4. **跨平台**——Windows 系统托盘，macOS 菜单栏图标
5. **所有数据存为本地文件**（JSON 存档 + 本地副本文件），不做云端数据库
...
| 副本抽取 | 从所有可用副本中随机选一个 |
| 导入副本 | 导入 `.md` 格式副本文件 |
| AI 生成副本 | 自备 API Key，AI 按模板生成 `.md` 副本 |
...
| `ai_output_language` | AI 生成副本时的输出语言 | `zh` |
...
| **v0.3.7** | **副本填充（`.md` 解析器 + 500 条事件 + 4 副本骨架）** | ⬜ |

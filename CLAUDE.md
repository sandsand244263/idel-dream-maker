...
| 事件文本 | 500 条/副本 | 线性人生故事，每级 1 条唯一事件 |
...
**当前阶段：** v2.4.0（已完成） — 数据规格对齐 + 节假日系统重写
...
| **2.4.0** | **2026-06-16** | **宠物优化：** 无宠物提示文字+导入后列表刷新+单击动画随机(非仅wave)+待机更活跃(20s/30%)
| **2.4.0** | **2026-06-16** | **精灵帧数自动检测：** 加载精灵图时检测每行有效帧数，兼容不同宠物实际帧数差异（如 review 行 5 帧而非固定 6 帧），消除因播放空白帧导致的闪烁

...
const LANG = {
  zh: {
    ...
    aliasTitle: '进入剧本', aliasPlaceholder: '输入名称（留空用默认）',
  ...
  en: {
    ...
    logStartHub: 'Idel-DreamMaker v0.3.1 ready',
...
aliasCancel.addEventListener('click', () => {
  aliasModal.classList.add('hidden');
  if (aliasResolver) { aliasResolver(null); aliasResolver = null; }
});
...
aliasInput.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') aliasConfirm.click();
  if (e.key === 'Escape') aliasCancel.click();
});

// ── Mini Bar ──

miniExpand.addEventListener('click', async () => {
  try { await invoke('set_window_mode', { mode: 'full' }); } catch (e) {}
  isMiniMode = false;
...
miniBar.addEventListener('mousedown', (e) => {
  ...
  const moveHandler = async (ev) => {
    if (!dragging) return;
    try { await invoke('set_window_position', { x: ev.screenX - offsetX, y: ev.screenY - offsetY }); } catch (ex) {}
  };
...
function updateMiniBar() {
  ...
  miniLevel.textContent = `Lv.${gameState.level}`;
...
async function init() {
  ...
  listen('event-triggered', (event) => {
    ...
    addLog('event', text); showEventOverlay(title, color, text);
  });
...
function renderHubView() {
  ...
  scenarioList.forEach(s => {
    ...
    card.addEventListener('click', async () => {
      const alias = await showAliasModal(s.nameCN);
...
btnScenario.addEventListener('click', async () => {
  try { scenarioList = await invoke('get_scenario_list'); } catch (e) {} renderScenarioPanel(); scenarioPanel.classList.remove('hidden');
});
...
settingsNameSave.addEventListener('click', async () => {
  ...
  try { await invoke('set_player_name', { name: n }); if (gameState) gameState.player_name = n; renderHubView(); } catch (e) {}
});
settingsTheme.addEventListener('change', async () => {
  const v = settingsTheme.value; try { await invoke('set_font_theme', { theme: v }); if (gameState) gameState.selected_font_theme = v; applyTheme(v); } catch (e) {}
});
settingsLanguage.addEventListener('change', async () => {
  const v = settingsLanguage.value; try { await invoke('set_language', { lang: v }); if (gameState) gameState.language = v; applyLanguage(); renderHubView(); } catch (e) {}
});
settingsAILanguage.addEventListener('change', async () => {
  const v = settingsAILanguage.value; try { await invoke('set_ai_output_language', { lang: v }); if (gameState) gameState.ai_output_language = v; } catch (e) {}
});
...
function renderScenarioPanel() {
  ...
  const cid = gameState?.scenario_id;
  scenarioList.forEach(s => {

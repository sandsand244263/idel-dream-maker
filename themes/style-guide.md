...
| 类别 | 变量 | 用途 | 示例 |
|:-----|:-----|:------|:------|
| **结构分隔线** | `var(--dim)` | 重要框架线，需保持对比度 | 主窗口外框、titlebar 底边、status bar 顶边、弹窗外框、按钮外框 |
| **装饰边框** | `var(--border)` | 组件内部边框，柔和区分 | 面板边框、卡片边框、列表项边框 |
| **强调边框** | `var(--fg)` | 需要视觉突出的边界 | 焦点状态、弹窗强调框、重要提示框 |
...
| 12 | 宠物通知圆点 |
| 13 | 宠物气泡区域 |
| 250 | 事件弹窗 overlay（临时反馈 > 常驻面板） |
| 251 | 成就弹窗 overlay（临时反馈 > 常驻面板） |
| 200 | 侧面板（称号/成就/设置/副本/事件/结局） |
| 300 | 下拉菜单（历程菜单）、别名弹窗 |
| 350 | 确认弹窗 |
| 400 | 引导弹窗 |
| 500 | Toast |
...
|:----|:--------|:------|
| `✕` | `\2715` | 关闭按钮 |
| `▶` | `\25B6` | 展开、可点击进入（上下文菜单、选择器、日期折叠） |
| `▼` | `\25BC` | 收起（已展开内容） |
| `✓` | `\2713` | 开关开启 |
| `✗` | `\2717` | 开关关闭 |

> 注意：Maple Mono NF CN 字体不支持 `◄`（`\25C4`）字符，所有导航/返回操作统一使用 `▶` 替代。

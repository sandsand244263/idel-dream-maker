...
- Mini Bar：v2.0 已替换为 PetDex 像素宠物窗口（独立 BrowserWindow，透明背景）
- 贴边自动隐藏：已移除（v0.3.2）
...
| 9. 宠物窗口 | **执行中** | v2.0 — PetDex 社区精灵 + Canvas 8×9 帧动画 |
| 10. 节假日系统 | **0%** | 待定 |
...
| **1.0-dev** | **2026-06-12** | **架构迁移计划：** Tauri → Electron。原因：WebView2 加载器与 Edge v149 不兼容。|
| **1.0.0** | **2026-06-12** | **Electron 迁移完成：** 8 个 Task 全部完成。 Rust 引擎重写为 JS (src/scenario.js, src/game.js)；electron/main.cjs 主进程；electron/tray.cjs/windows.cjs 模块化；build.js 替代 build.rs；删除全部 Tauri 文件 |
| **2.0.0-dev** | **2026-06-12** | **像素宠物窗口启动：** PetDex 社区精灵集成（手动下载 + 本地加载）；独立 BrowserWindow 替换 Mini Bar；Canvas 8×9 帧动画渲染 |
